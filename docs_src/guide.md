---
title: User Guide - JereIDE
description: Keyboard shortcuts, LSP language server setup, configuration, and syntax highlighting for JereIDE.
---
...
JereIDE includes built-in configurations for the following language servers. Install the binary and it works automatically -- no configuration needed.
...
JereIDE is configured via a TOML file. Open it from the sidebar settings icon or via the command palette: **Open User Settings**.

### Config location

| Platform | Path |
|----------|------|
| Linux | `~/.config/jereide/config.toml` |
| macOS | `~/Library/Application Support/jereide/config.toml` |
| Windows | `%APPDATA%\jereide\config.toml` |
...
```
jereide [file...]          Open files
jereide file.rs:42         Open file at line 42
jereide -v                 Verbose mode (log LSP errors to stderr)
