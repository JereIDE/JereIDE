...
# Produces:
#   dist/JereIDE.app                                  (codesigned ad-hoc, xattrs cleared)
#   dist/jereide-${VERSION}-macos-${ARCH}.zip         (release archive)
#
...
DIST_DIR="dist"
APP="$DIST_DIR/JereIDE.app"
ARCHIVE="$DIST_DIR/jereide-${VERSION}-macos-${ARCH_LABEL}.zip"
...
bundle_macos_dylibs() {
    local app="$1"
    local binary="$app/Contents/MacOS/jereide"
...
sign_macos_app() {
    ...
    codesign --force --sign - --timestamp=none "$app/Contents/MacOS/jereide"
...
BINARY="target/$RUST_TARGET/release/jereide"
...
cp "$BINARY" "$APP/Contents/MacOS/jereide"
chmod 755 "$APP/Contents/MacOS/jereide"
...
(cd "$DIST_DIR" && zip -qry "$(basename "$ARCHIVE")" JereIDE.app install-mac.sh)
