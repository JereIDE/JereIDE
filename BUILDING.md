...
```bash
# One-shot.
SDL_VIDEO_DRIVER=wayland jereide
...
The binary is `target/release/jereide`.
...
```
cp target/release/jereide ~/.local/bin/
cp -r data ~/.local/share/jereide/
```

To register for "Open With" on supported file types:

```
cp resources/linux/com.jeremy.jereide.desktop ~/.local/share/applications/
cp resources/icons/jereide.png ~/.local/share/icons/hicolor/128x128/apps/
...
Move `JereIDE.app` to `/Applications`. The Info.plist registers JereIDE
...
```bash
sudo xattr -dr com.apple.quarantine /Applications/JereIDE.app
```

### Windows

Copy `jereide.exe` and the `data/` directory wherever you like, then
