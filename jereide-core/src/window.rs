...
// App metadata passed to SDL_SetAppMetadata in `init`.
thread_local! {
    static APP_NAME: std::cell::RefCell<std::ffi::CString> =
        std::cell::RefCell::new(
            std::ffi::CString::new("JereIDE").expect("static app name"),
        );
    static APP_IDENTIFIER: std::cell::RefCell<std::ffi::CString> =
        std::cell::RefCell::new(
            std::ffi::CString::new("jereide").expect("static app id"),
        );
}
...
// Bytes of the application icon used by `set_window_icon`.
thread_local! {
    static APP_ICON_BYTES: std::cell::RefCell<&'static [u8]> = const {
        std::cell::RefCell::new(include_bytes!("../../resources/icons/jereide.png"))
    };
}
